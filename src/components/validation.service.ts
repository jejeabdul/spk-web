export class ValidationService {
    ...
    static phoneValidator(control): any {
        ...
        // (?=.*[0-9])       - Assert a string has at least one number
        if (control.value) {
            if ((control.value.match(/^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{5})$/)) || (control.value.match(/^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{6})$/)) || (control.value.match(/^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{7})$/)) || (control.value.match(/^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{8})$/)) || (control.value.match(/^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{9})$/))) {
                return null;
            } else {
